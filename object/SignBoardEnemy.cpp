...
SignBoardEnemy::SignBoardEnemy(const int handle, const Material materialType, const LoadObjectInfo objInfo):EnemyBase(handle,materialType,objInfo)
{
	...
	numModel_[0] = MV1DuplicateModel(model.GetModelHandle(objData[static_cast<int>(ObjectType::Number)].name));
	numModel_[1] = MV1DuplicateModel(model.GetModelHandle(objData[static_cast<int>(ObjectType::Number)].name));
}
